...
export function MobileBottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-slate/10 bg-white/95 backdrop-blur-md md:hidden">
      <div className="flex items-center justify-between px-1 py-1.5 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
        {TABS.map((tab) => {
          ...
          return (
            <Link
              ...
              href={tab.href}
              className={cn(
                "relative flex min-w-0 flex-1 flex-col items-center gap-1 rounded-lg px-1 py-1.5 text-[10px] font-medium transition-colors",
            ...
            >
              ...
              {tab.href === "/shortlist" && shortlistHydrated && shortlistCount > 0 && (
                <span className="absolute right-3 top-0 flex h-4 w-4 items-center justify-center rounded-full bg-burgundy text-[9px] font-bold text-white">
                  {shortlistCount}
                </span>
              )}
              {tab.href === "/booking/cart" && hydrated && items.length > 0 && (
                <span className="absolute right-3 top-0 flex h-4 w-4 items-center justify-center rounded-full bg-gold text-[9px] font-bold text-burgundy-deep">
                  {items.length}
                </span>
              )}
            </Link>
